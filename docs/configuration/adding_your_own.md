# peterk87/nf-villumina: Configuration for other clusters
...
Nextflow has [excellent integration](https://www.nextflow.io/docs/latest/docker.html) with Docker, and beyond installing the two tools, not much else is required - nextflow will automatically fetch the [nfcore/ontasfv](https://hub.docker.com/r/peterk87/nf-villumina/) image that we have created and is hosted at dockerhub at run time.

To add docker support to your own config file, add the following:

```nextflow
docker.enabled = true
process.container = "peterk87/nf-villumina"
```
...
process.container = "shub://peterk87/nf-villumina"
```
...
```nextflow
singularity.enabled = true
process.container = "/path/to/nf-villumina.simg"
```
