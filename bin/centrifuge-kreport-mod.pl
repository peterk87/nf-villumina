...
#
# Modified for reading previously output taxonomy tree and taxid to name table by Peter Kruczkiewicz
...
my ($names, $taxtree, $min_score, $min_length);
my $no_lca = 0;
my $show_zeros = 0;
my $is_cnts_table = 0;
my $PROG = "centrifuge-kreport-mod";
...
usage() unless ((defined $names) && (defined $taxtree));
if (!defined $ARGV[0]) {
...
sub usage {
  ...
  print STDERR "
Usage: centrifuge-kreport-mod.pl --names NAMES --taxtree TAXTREE OPTIONS <centrifuge output file(s)>

centrifuge-kreport creates Kraken-style reports from centrifuge out files.

Options:
    --names NAMES        (REQUIRED) Centrifuge index name table from `centrifuge-inspect --name-table NAMES <centrifuge index>`
    --taxtree TAXTREE    (REQUIRED) Centrifuge index taxonomy tree from `centrifuge-inspect --taxonomy-tree TAXTREE <centrifuge index>` 
...
sub load_taxonomy {
  ...
  open(NAMES, "<", $names) or die "$PROG: can't open names file: $!\n";
  while (<NAMES>) {
    chomp;
    s/\t\|$//;
    my @fields = split /\t/;
  ...
  open(NODES, "<", $taxtree) or die "$PROG: can't open taxtree '$taxtree' file: $!\n";
  while (<NODES>) {
    chomp;
    my @fields = split /\t\|\t/;
